...
contract Desafio_4 {
    ...
    //mapping(bytes32 => bool) public listaSubastasActivas;

    bytes32[] arrSubastasActivas;
    ...
    function creaSubasta(uint256 _startTime, uint256 _endTime) public payable {
        ...
        //Auction storage auction = Auction(_auctionId, true, _startTime, _endTime, 0, 0);
        ...
        arrSubastasActivas.push(_auctionId);
        //listaSubastasActivas[_auctionId] = true;
    ...
    function proponerOferta(bytes32 _auctionId) public payable {
        ...
        //if(!listaSubastasActivas[_auctionId]){
        if(!auction.activo){
        ...
        //(+=) las ofertas son independientes, pero se va acumulando el total de ofertas, eso servira para la devolucion total
    ...
    function finalizarSubasta(bytes32 _auctionId) public {
        ...
        //delete listaSubastasActivas[_auctionId];//delete auctions[_auctionId];//delete auction;
        for (uint256 i; i < arrSubastasActivas.length; i++) {
            if(arrSubastasActivas[i] == _auctionId){
                arrSubastasActivas[i] = arrSubastasActivas[arrSubastasActivas.length - 1];
                arrSubastasActivas.pop();
            }
        }

        auction.activo = false;
    ...
    function recuperarOferta(bytes32 _auctionId) public {
        ...
        payable(msg.sender).transfer(auctions[_auctionId].offers[msg.sender]);
    }

    function verSubastasActivas() public view returns (bytes32[] memory) {
        return arrSubastasActivas;
    }
